...
try {
    $stmt = $conexion->prepare('
        SELECT c.id_usuario, c.tmdb_id, c.nombre_pelicula, c.comentario, c.valoracion, c.fecha_comentario, u.nombre_usuario
        FROM comentarios c
    ...
    ');
    $stmt->execute();
    $todos_los_comentarios = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Filtrar por tmdb_id si hay búsqueda (opcional)
    if (!empty($search_query)) {
        $comentarios_filtrados = array_filter($todos_los_comentarios, function($comentario) use ($search_query) {
            return stripos($comentario['tmdb_id'], $search_query) !== false;
        });
    } else {
        $comentarios_filtrados = $todos_los_comentarios;
    }
    ...
    <main class="contenido">
        <section class="comentarios-section">
            ...
            <form action="lista-comentarios.php" method="GET" class="barra-busqueda-comentarios">
                <input type="text" name="search" placeholder="Buscar por ID de película..." value="<?php echo htmlspecialchars($search_query); ?>">
                <button type="submit" class="btn-buscar-comentarios">Buscar</button>
            </form>

            <?php if (!empty($error)): ?>
                <div class="mensaje-error"><?php echo $error; ?></div>
            <?php elseif (empty($comentarios_filtrados)): ?>
                <p class="mensaje-sin-comentarios">No se encontraron comentarios.</p>
            <?php else: ?>
                <div id="comentarios-container" class="lista-comentarios-grid">
                    <?php foreach ($comentarios_filtrados as $comentario): ?>
